...
describe('UsernameEntry.vue', () => {
  ...
  it('validates whitespace-only username', async () => {
    ...
    expect(wrapper.find('.form-error').text()).toContain('Username cannot contain only spaces');
  });
  ...
  it('validates username length', async () => {
    ...
    expect(wrapper.find('.form-error').text()).toContain('Username must be 50 characters or less');
  });
