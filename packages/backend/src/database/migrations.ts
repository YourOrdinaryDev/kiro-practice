...
export class DatabaseMigrations {
  ...
  private async createDefaultList(userId: number): Promise<{ id: number; name: string }> {
    ...
    console.log(`Created default list "${defaultListName}" with ID: ${result.lastID}`);
    return { id: result.lastID!, name: defaultListName };
  }

  private async migrateExistingTodos(existingTodos: any[], listId: number): Promise<void> {
