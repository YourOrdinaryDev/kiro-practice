...
async function testSessionIntegration() {
  ...
  try {
    ...
    if (lists.length > 0) {
      ...
      const todo = await todoService.createTodoInList(
        defaultList.id,
        'Test todo for session integration'
      );
      console.log(
        `✓ Created todo in user's list: "${todo.text}" (ID: ${todo.id})`
      );
    ...
    const anotherUserLists = await todoListService.getListsForUser(
      anotherUser.id
    );

    console.log(`✓ First user (${user.username}) has ${lists.length} lists`);
    console.log(
      `✓ Second user (${anotherUser.username}) has ${anotherUserLists.length} lists`
    );
