...
describe('TodoListService', () => {
  ...
  describe('getListsForUser', () => {
    ...
    it('should reject invalid user ID', async () => {
      await expect(todoListService.getListsForUser(-1)).rejects.toThrow(
        'User ID must be a positive integer'
      );
      await expect(todoListService.getListsForUser(0)).rejects.toThrow(
        'User ID must be a positive integer'
      );
    });
  });
  ...
  describe('createList', () => {
    ...
    it('should reject empty list name', async () => {
      await expect(todoListService.createList(testUserId, '')).rejects.toThrow(
        'List name cannot be empty or contain only whitespace'
      );
      await expect(
        todoListService.createList(testUserId, '   ')
      ).rejects.toThrow('List name cannot be empty or contain only whitespace');
    });
    ...
    it('should reject duplicate list names for same user', async () => {
      ...
      await expect(
        todoListService.createList(testUserId, listName)
      ).rejects.toThrow(
        `A list named "${listName}" already exists for this user`
      );
    });
    ...
    it('should reject invalid user ID', async () => {
      await expect(todoListService.createList(-1, 'Test List')).rejects.toThrow(
        'User ID must be a positive integer'
      );
      await expect(todoListService.createList(0, 'Test List')).rejects.toThrow(
        'User ID must be a positive integer'
      );
    });
    ...
    it('should reject non-existent user ID', async () => {
      await expect(
        todoListService.createList(99999, 'Test List')
    ...
    it('should reject list name longer than 100 characters', async () => {
      ...
      await expect(
        todoListService.createList(testUserId, longName)
      ).rejects.toThrow('List name cannot exceed 100 characters');
    });

    it('should trim whitespace from list name', async () => {
      const list = await todoListService.createList(
        testUserId,
        '  Trimmed List  '
      );
  ...
  describe('updateListName', () => {
    ...
    it('should reject empty new name', async () => {
      await expect(todoListService.updateListName(listId, '')).rejects.toThrow(
        'List name cannot be empty or contain only whitespace'
      );
      await expect(
        todoListService.updateListName(listId, '   ')
      ).rejects.toThrow('List name cannot be empty or contain only whitespace');
    });
    ...
    it('should reject duplicate name for same user', async () => {
      ...
      await expect(
        todoListService.updateListName(listId, 'Existing Name')
      ).rejects.toThrow(
        'A list named "Existing Name" already exists for this user'
      );
    });

    it('should allow updating to same name (no change)', async () => {
      const originalName = 'Original Name';
      const updatedList = await todoListService.updateListName(
        listId,
        originalName
      );
    ...
    it('should validate ownership when userId provided', async () => {
      ...
      await expect(
        todoListService.updateListName(listId, 'New Name', otherUser.id)
      ).rejects.toThrow(`List with ID ${listId} not found`);
    });

    it('should reject invalid list ID', async () => {
      await expect(
        todoListService.updateListName(-1, 'New Name')
      ).rejects.toThrow('List ID must be a positive integer');
      await expect(
        todoListService.updateListName(0, 'New Name')
      ).rejects.toThrow('List ID must be a positive integer');
    });

    it('should reject non-existent list ID', async () => {
      await expect(
        todoListService.updateListName(99999, 'New Name')
      ).rejects.toThrow('List with ID 99999 not found');
    });
  });
  ...
  describe('deleteList', () => {
    ...
    it('should prevent deleting last remaining list', async () => {
      ...
      await expect(todoListService.deleteList(defaultListId)).rejects.toThrow(
    ...
    it('should validate ownership when userId provided', async () => {
      ...
      await expect(
        todoListService.deleteList(listId, otherUser.id)
      ).rejects.toThrow(`List with ID ${listId} not found`);
    });

    it('should reject invalid list ID', async () => {
      await expect(todoListService.deleteList(-1)).rejects.toThrow(
        'List ID must be a positive integer'
      );
      await expect(todoListService.deleteList(0)).rejects.toThrow(
        'List ID must be a positive integer'
      );
    });

    it('should reject non-existent list ID', async () => {
      await expect(todoListService.deleteList(99999)).rejects.toThrow(
        'List with ID 99999 not found'
      );
    });
  });
  ...
  describe('validateListOwnership', () => {
    ...
    it('should return true for valid ownership', async () => {
      const isOwner = await todoListService.validateListOwnership(
        listId,
        testUserId
      );
    ...
    it('should reject invalid IDs', async () => {
      await expect(
        todoListService.validateListOwnership(-1, testUserId)
      ).rejects.toThrow('List ID must be a positive integer');
      await expect(
        todoListService.validateListOwnership(listId, -1)
  ...
  describe('getListById', () => {
    ...
    it('should reject invalid list ID', async () => {
      await expect(todoListService.getListById(-1)).rejects.toThrow(
        'List ID must be a positive integer'
      );
      await expect(todoListService.getListById(0)).rejects.toThrow(
        'List ID must be a positive integer'
      );
    });

    it('should reject invalid user ID when provided', async () => {
      await expect(todoListService.getListById(listId, -1)).rejects.toThrow(
        'User ID must be a positive integer'
      );
      await expect(todoListService.getListById(listId, 0)).rejects.toThrow(
        'User ID must be a positive integer'
      );
    });
  });
