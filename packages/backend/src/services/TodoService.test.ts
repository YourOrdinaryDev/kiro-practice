...
describe('TodoService', () => {
  ...
  describe('createTodo', () => {
    ...
    it('should reject empty text', async () => {
      ...
      await expect(
        todoService.createTodo(testUsername, todoData)
      ).rejects.toThrow('Todo text cannot be empty or contain only whitespace');
    });
  ...
  describe('getAllTodos', () => {
    ...
    it('should return only todos for the specific user', async () => {
      ...
      const todo1 = await todoService.createTodo(testUsername, {
        text: 'User1 todo',
      });
      const todo2 = await todoService.createTodo('otheruser', {
        text: 'User2 todo',
      });
    ...
    it('should return all todos for user ordered by completion status', async () => {
      ...
      const todo2 = await todoService.createTodo(testUsername, {
        text: 'Second todo',
      });
  ...
  describe('updateTodo', () => {
    it('should update todo completion status for owned todo', async () => {
      const createdTodo = await todoService.createTodo(testUsername, {
        text: 'Test todo',
      });
    ...
    it('should reject updating todo owned by different user', async () => {
      const createdTodo = await todoService.createTodo('otheruser', {
        text: 'Other user todo',
      });

      await expect(
        todoService.updateTodo(testUsername, createdTodo.id, {
          completed: true,
        })
      ).rejects.toThrow(`Todo with ID ${createdTodo.id} not found`);
    });

    it('should reject invalid todo ID', async () => {
      await expect(
        todoService.updateTodo(testUsername, -1, { completed: true })
      ).rejects.toThrow('Todo ID must be a positive integer');
    });

    it('should reject non-existent todo ID', async () => {
      await expect(
        todoService.updateTodo(testUsername, 999, { completed: true })
  ...
  describe('deleteTodo', () => {
    ...
    it('should reject deleting todo owned by different user', async () => {
      ...
      await expect(
        todoService.deleteTodo(testUsername, createdTodo.id)
      ).rejects.toThrow(`Todo with ID ${createdTodo.id} not found`);
    });
  ...
  describe('List-based operations (new schema)', () => {
    ...
    describe('getTodosInList', () => {
      ...
      it('should return only todos from the specified list', async () => {
        ...
        const todo2 = await todoService.createTodoInList(
          listId2,
          'Personal todo'
        );
    ...
    describe('createTodoInList', () => {
      ...
      it('should reject non-existent list', async () => {
        await expect(
          todoService.createTodoInList(999, 'Test todo')
    ...
    describe('moveTodoToList', () => {
      it('should move a todo from one list to another', async () => {
        ...
        const originalTodo = await todoService.createTodoInList(
          listId1,
          'Movable todo'
        );

        // Move it to list2
        const movedTodo = await todoService.moveTodoToList(
          originalTodo.id,
          listId2,
          userId
        );
      ...
      it('should preserve completion status when moving', async () => {
        ...
        await todoService.updateTodo(testUsername, originalTodo.id, {
          completed: true,
        });
      ...
      it('should reject moving to non-existent list', async () => {
        ...
        await expect(
          todoService.moveTodoToList(todo.id, 999, userId)
        ).rejects.toThrow('Target list not found or does not belong to user');
      });

      it('should reject moving non-existent todo', async () => {
        await expect(
          todoService.moveTodoToList(999, listId2, userId)
        ).rejects.toThrow('Todo not found or does not belong to user');
      });
      ...
      it('should reject moving todo to list owned by different user', async () => {
        ...
        const otherList = await todoListService.createList(
          otherUser.id,
          'Other List'
        );
        ...
        await expect(
          todoService.moveTodoToList(todo.id, otherList.id, userId)
        ).rejects.toThrow('Target list not found or does not belong to user');
      });
    });
    ...
    describe('createTodo with list_id', () => {
      ...
      it('should reject creating todo in list owned by different user', async () => {
        ...
        await expect(
          todoService.createTodo(testUsername, {
            text: 'Test todo',
